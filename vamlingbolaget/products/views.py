...
def cutondemandApi(request, category): 
    ...
    children = Variation.objects.filter(category__slug='barn', order__lte=100, active=True).order_by('order', 'article__quality')
    print children
    accessories = Variation.objects.filter(category__slug='accessoarer', order__lte=100, active=True).order_by('order', 'article__quality')
